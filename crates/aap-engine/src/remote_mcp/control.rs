...
impl Session {
    pub(super) async fn reply_remote_ping(
        ...
        let binding = &parent
            ...
            .binding;
        let mut child = parent.child(ExecuteRequest {
            ...
            body_base64: STANDARD.encode(SAFE_PING_REPLY),
        })?;
        ...
        let attempt = async {
            ...
            *outgoing.headers_mut() = prepared.headers;
            ...
            child.content_both(Direction::Outbound, SAFE_PING_REPLY)?;
            child.record_both(
                Direction::Outbound,
                Data::AuthTransition {
        ...
        let result = tokio::select! {
            ...
            _ = parent.operation.cancelled.cancelled() => Err(ErrorCode::RequestConflict.into()),
